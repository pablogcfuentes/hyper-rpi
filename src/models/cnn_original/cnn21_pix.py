...
# oitaven, 200 muestras, patches 28x28, N2=16, N3=32, D1=2, D2=2
# CNN21 PIX EXP: 5 EPOCHS: 100 SAMPLES: [0.02, 0.0] ADA: 0 AUM: 0
# Class 01: 98.99+0.73
# Class 02: 99.26+0.80
# Class 03: 99.90+0.14
# Class 04: 99.80+0.26
...
def main(exp):
  ...
  for epoch in range(EPOCHS):
    for i,(inputs,labels) in enumerate(train_loader):
      # 7.1. Cogemos muestras y etiquetas para entrenar
      ...
      # 7.3.1. reset the gradients (PyTorch accumulates gradients)
      optimizer.zero_grad()
  ...
  # 9. precisiones a nivel de clase
  ...
  # 10. guardamos la salida
  if(PAD):
     output=np.reshape(output,(-1,H1))
     output=output[sizey//2:V1-sizey//2,sizex//2:H1-sizex//2]
     H1=H1-2*(sizex//2); V1=V1-2*(sizey//2)
     output=np.reshape(output,(H1*V1))
  save_pgm(output,H1,V1,nclases,'/tmp/output_cnn21-'+str(exp)+'.pgm')
  # Save the model checkpoint
  torch.save(model.state_dict(),'/tmp/model_cnn21.ckpt')
...
if __name__=='__main__':
  ...
  print('* CNN21 PIX EXP:',EXP,'EPOCHS:',EPOCHS,'SAMPLES:',SAMPLES,'ADA:',ADA,'AUM:',AUM)
