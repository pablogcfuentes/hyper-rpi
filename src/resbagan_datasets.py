...
AVAIL_DATASETS = {
    ...
    "pavia_university": {
        ...
        "gt": [
            "oitaven_river_converted_gt.raw",
        ],
        "segmentation": [
            "seg_oitaven_wp.raw",
        ],
...
class HyperDataset(torch.utils.data.Dataset):
    ...
    ordered_test_set : dict
        Dictionary that contains the ordered test set of the dataset. It has two keys: "samples" and "labels".
    ...
    def split_dataset(self, ratios):
        ...
        self.ordered_test_set  = { "samples": [], "labels": [] }
        ...
        for class_i, samples in enumerate(all_samples):
            ...
            # Modification to create a ordered set of samples
            ordered_test = samples[int(len(samples) * (train_ratio + val_ratio)) :]
            self.ordered_test_set["samples"] += ordered_test
            self.ordered_test_set["labels"] += [class_i] * len(ordered_test)
        ...
        # Modification to obtain the indexes array
        self.test_index_list = []
        if not self.segmented:
            for sample in self.ordered_test_set["samples"]:
                row, col = sample
...
def read_unprocessed_dataset(name):
    ...
    if AVAIL_DATASETS[name]["format"] == "raw":
        ...
        with open(f"../data/imagenes_rios/{AVAIL_DATASETS[name]['image'][0]}", "rb") as input:
        ...
        with open(f"../data/imagenes_rios/{AVAIL_DATASETS[name]['gt'][0]}", "rb") as input:
